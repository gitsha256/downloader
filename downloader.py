...
import subprocess
import traceback
import sys  # Added for the fix
import io   # Added for the fix

# --- Logging ---
# FIX: In a windowed app, stdout/stderr may be None. Redirect logging to a file.
...
async def download_manager_async(url, output_file, gui):
    ...
    try:
        async with aiohttp.ClientSession(
            ...
            # FIX: Tell tqdm to write to a dummy in-memory text buffer instead of the console
...
def add_to_queue(entry, listbox):
    ...
    if not url:
        ...
        return
    try:
        # A new loop for this small, one-off async task is fine.
...
def main():
    ...
    for name in ['add.png', 'start.png', 'pause.png', 'stop.png', 'remove.png','download.png']:
        try:
            # This needs to be adjusted for PyInstaller's temp directory
            base_path = getattr(sys, '_MEIPASS', os.path.dirname(os.path.abspath(__file__)))
            icons[name] = ImageTk.PhotoImage(Image.open(os.path.join(base_path, name)).resize((20, 20)))
        except Exception as e:
            logging.error(f"Could not load icon {name}: {e}")
            icons[name] = None
...
if __name__ == "__main__":
    # Small correction for resolving URL without a running event loop in add_to_queue
    # This block is now better structured for sync/async interaction
    main()